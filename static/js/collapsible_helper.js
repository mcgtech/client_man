$(function(){
    setup_collapsible($("#search_filters"), $("#collapse_butt"));
...
function setup_collapsible(target_block, button)
{
      target_block.on("hide.bs.collapse", function(){
        button.html('<span class="glyphicon glyphicon-collapse-down"></span> Open filters');
      ...
      target_block.on("show.bs.collapse", function(){
            button.html('<span class="glyphicon glyphicon-collapse-up"></span> Close filters');
            var active = $(this).attr('id');
            var blocks= localStorage.blocks === undefined ? new Array() : JSON.parse(localStorage.blocks);
