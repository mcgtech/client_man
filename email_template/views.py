...
def manage_email_temp(request, temp_id=None):
    ...
    if temp_id is None:
        ...
        msg_once_only(request, 'Adding a new email template', settings.WARN_MSG_TYPE)
    else:
    ...
    if request.method == "POST":
        if request.POST.get("delete-record"):
            msg_once_only(request, 'You have successfully deleted the email template ' + str(temp), settings.INFO_MSG_TYPE)
            temp.delete()
